...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('[KB Approve] Executing UPDATE...');
    ...
    if (!updateResponse.ok) {
      ...
      console.error('[KB Approve] Update failed:', errorData);
      throw new Error(`Update failed: ${errorData.message || updateResponse.statusText}`);
    }

    const updateResult = await updateResponse.json();
    console.log('[KB Approve] Update successful');

    // Step 2: Verify the update actually worked by reading back the file
    const verifySQL = `
    ...
    console.log('[KB Approve] Verifying approval status...');
    ...
    if (!verifyResponse.ok) {
      ...
      console.error('[KB Approve] Verification failed:', errorData);
    ...
    if (rows.length === 0) {
      console.error('[KB Approve] File not found after update');
      return res.status(404).json({ error: 'File not found' });
    }

    const [verifiedFileId, fileName, isApproved, approverEmail, approvalDate] = rows[0];

    // Confirm the DB actually shows TRUE
    if (isApproved !== true) {
      console.error('[KB Approve] Approval did not persist to database:', { isApproved });
      throw new Error('Approval did not persist to database - please try again');
    }

    console.log(`[KB Approve] SUCCESS: ${fileName} approved by ${userEmail} at ${approvalDate}`);
    console.log(`[KB Approve] Verified: isApproved=${isApproved}, approverEmail=${approverEmail}`);
    ...
    return res.status(200).json({
      ...
      verified: true, // Indicates we verified the DB update
    });
